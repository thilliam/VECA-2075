...
function addSettlements(data){
 ...
 levels.forEach(x=>{
  map.addLayer({id:`settlements-r${x.r}-dots`,type:'circle',source:'settlements',minzoom:x.z,filter:['==',['get','settlement_rank'],x.r],layout:{visibility:state.settlements?'visible':'none'},paint:{'circle-radius':x.d,'circle-color':'#ffffff','circle-opacity':0.92,'circle-stroke-color':'#17232b','circle-stroke-width':2}});
  map.addLayer({id:`settlements-r${x.r}`,type:'symbol',source:'settlements',minzoom:x.z,filter:['==',['get','settlement_rank'],x.r],layout:{visibility:state.settlements?'visible':'none','text-field':['get','name'],'text-size':x.s,'text-offset':[0,1.25],'text-anchor':'top','text-allow-overlap':false,'text-padding':3},paint:{'text-color':'#ffffff','text-halo-color':'#17232b','text-halo-width':2}});
  wireClick(`settlements-r${x.r}-dots`);wireClick(`settlements-r${x.r}`);
 });
}
...
function updateLegend(){let html='<div class="legend-title">Map legend</div>';
 if(state.growth){html+='<div class="legend-sub">Population change, 2020–25</div>'+swatch('#5276a7','Decline ~−10%')+swatch('#d9d9d9','Little / no change')+swatch('#e9b44c','Growth ~10%')+swatch('#d95f36','Growth ~30%')+swatch('#a20d2a','Very high growth ~70%+');}
 else if(state.population){html+='<div class="legend-sub">2025 population density · people/km²</div>'+swatch('#0a1720','Very sparse · <10')+swatch('#173a4c','Sparse · 10–100')+swatch('#277d86','Urbanising · 100–500')+swatch('#e1bb62','Urban · 500–2,000')+swatch('#e56b4f','Dense · 2,000–6,000')+swatch('#d32929','Very dense · 6,000+');}
 if(state.settlements){html+='<div class="legend-divider"></div><div class="legend-sub">Settlement hierarchy · appears progressively with zoom</div>'+dot(18,'Rank 1 · major metropolis')+dot(16,'Rank 2 · large urban centre')+dot(14,'Rank 3 · regional / metro centre')+dot(12,'Rank 4 · town / small city')+dot(10,'Ranks 5–6 · smaller towns');}
 if(state.landUse){html+='<div class="legend-divider"></div><div class="legend-sub">Land use</div><div class="note">Colours are supplied by the live ABARES 2020–21 national land-use service. This POC does not yet reproduce its full categorical key.</div>';}
 document.getElementById('legendPanel').innerHTML=html;
}
async function load(){try{
 const [pop,settlements,rail,roads]=await Promise.all([fetch('./data/population_sa2.geojson').then(r=>r.json()),fetch('./data/settlements.geojson').then(r=>r.json()),fetch('../poc-002/data/rail.geojson').then(r=>r.json()),fetch('../poc-002/data/roads.geojson').then(r=>r.json())]);
 addRaster();addPopulation(pop);addTransport('rail',rail,'#6fd8ff',4,true);addTransport('roads',roads,'#d7ba74',5,false);addSettlements(settlements);updateLegend();
...
document.getElementById('population').addEventListener('change',e=>{state.population=e.target.checked;setVisible('population-fill',state.population&&!state.growth);setVisible('population-outline',state.population);updateLegend()});
document.getElementById('growth').addEventListener('change',e=>{state.growth=e.target.checked;setVisible('growth-fill',state.growth);setVisible('population-fill',state.population&&!state.growth);updateLegend()});
...
document.getElementById('closeDetail').addEventListener('click',()=>{document.getElementById('detailContent').innerHTML='<div class="eyebrow">MAP KEY</div><h2>Population & place</h2><p>Click an SA2 or settlement marker for population, growth and provenance.</p>';updateLegend()});