...
Three important direct imports are not spatially ready:
...
Open `http://localhost:8000/maps/poc-005/`.

## Geometry doctrine

Every new POC-005 feature carries a `geometry_quality` and explanatory `geometry_note`. Current geometry classes include `representative_locality_anchor`, `generalised_endpoint_corridor`, `generalised_corridor`, `representative_port_anchor`, `representative_region_anchor` and `representative_system_anchor`.

These are visual/analytical context until authoritative geometry replaces them.

## Spatial readiness

`assurance/spatial_readiness.json` is the machine-readable bridge between dataset arrival and map readiness. It distinguishes mapped datasets, map-ready datasets, datasets blocked on a spatial join and non-spatial evidence. `tools/validate_spatial_readiness.py` checks that mapped readiness records have catalogue entries and that referenced datasets exist.
