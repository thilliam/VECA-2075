...
present, then mirrored into POC-005 so prior layers remain available while new
system-capacity/digital layers are added.
"""
...
def ensure_poc004():
    ...
    for p in POC004.glob('*.json'):
        if p.name!='manifest.json':shutil.copy2(p,OUT/p.name)
