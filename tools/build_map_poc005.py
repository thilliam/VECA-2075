...
POC-005 is additive over POC-004. Existing POC-004 derivatives are reused if
present, then mirrored into POC-005 so the same frontend can retain all prior
layers while new system-capacity/digital layers are added.
"""
...
OUT=ROOT/'maps'/'poc-005'/'data'; POC004=ROOT/'maps'/'poc-004'/'data'
...
LOCALITIES={'Macquarie Park':(151.123,-33.777),'Artarmon':(151.185,-33.808),'Horsley Park':(150.856,-33.843),'Eastern Creek':(150.795,-33.806),'Sydney West':(150.82,-33.80),'Sydney North':(151.15,-33.72),'Marsden Park':(150.84,-33.70),'Port Melbourne':(144.94,-37.84),'Tullamarine':(144.88,-37.69),'West Footscray':(144.87,-37.80),'Melbourne West':(144.80,-37.82),'Melbourne North-West':(144.82,-37.70),'Brooklyn':(144.84,-37.82),'Laverton':(144.78,-37.86),'Geelong':(144.36,-38.15),'Brisbane CBD':(153.03,-27.47),'Fortitude Valley':(153.04,-27.46),'Gold Coast':(153.40,-28.02),'Bruce':(149.09,-35.24),'Hume':(149.17,-35.39),'Fyshwick':(149.16,-35.33),'Beard':(149.22,-35.35),'Maroochydore':(153.09,-26.65),'Sunshine Coast':(153.05,-26.65),'Sydney':(151.21,-33.87),'Canberra':(149.13,-35.28),'Melbourne':(144.96,-37.81),'Wollongong':(150.89,-34.43),'Newcastle':(151.78,-32.93),'Albury':(146.92,-36.08),'Wagga Wagga':(147.37,-35.12),'Parkes':(148.18,-33.14),'Narromine':(148.24,-32.23),'Narrabri':(149.78,-30.33),'North Star':(150.39,-28.93),'Goulburn':(149.72,-34.75),'Telarah':(151.54,-32.72),'Acacia Ridge':(153.00,-27.58),'Beveridge':(144.98,-37.48),'Illabo':(147.75,-34.98),'Stockinbingal':(147.88,-34.50),'Gowrie':(151.88,-27.53),'Helidon':(152.13,-27.55),'Calvert':(152.52,-27.67),'Fisherman Islands / Brisbane':(153.18,-27.38),'Wollongong / Illawarra':(150.90,-34.43),'Newcastle / Hunter':(151.78,-32.93)}
...
def point(eid,name,domain,lonlat,props,quality,note):
    p=dict(props);p.update({'entity_id':eid,'name':name,'domain':domain,'geometry_quality':quality,'geometry_note':note});return {'type':'Feature','geometry':{'type':'Point','coordinates':[lonlat[0],lonlat[1]]},'properties':p}
def line(eid,name,domain,coords,props,quality,note):
    p=dict(props);p.update({'entity_id':eid,'name':name,'domain':domain,'geometry_quality':quality,'geometry_note':note});return {'type':'Feature','geometry':{'type':'LineString','coordinates':[[x,y] for x,y in coords]},'properties':p}
def write(name,data):
...
def build_digital():
    ...
    for r in rows(DIGITAL):
        pts=[]
        for name in r.get('endpoints','').replace('–','-').split('-'):
            if name.strip() in CITY:pts.append(CITY[name.strip()])
        if len(pts)<2:missing.append(r['route_id']);continue
        out.append(line(r['route_id'],r['route_name'],'digital-connectivity',pts,{**r,'source_dataset':str(DIGITAL.relative_to(ROOT))},'generalised_endpoint_corridor','Generalised endpoint corridor; not fibre alignment or access-point geometry'))
    return fc(out),missing
...
def build_rail():
    ...
    for r in rows(RAIL):
        ...
        out.append(line(r['rail_signal_id'],r['program_or_corridor'],'rail-capacity-investment',pts,{**r,'source_dataset':str(RAIL.relative_to(ROOT))},'generalised_corridor','Generalised named corridor; not project alignment or engineering geometry'))
    return fc(out),missing
...
def build_dnsp():
    ...
    for r in rows(DNSP):
        ...
        out.append(point(r['signal_id'],r['asset_name'],'distribution-capacity-signal',loc,{**r,'source_dataset':str(DNSP.relative_to(ROOT))},'representative_region_anchor','Regional anchor for curated DNSP signal; not authoritative substation coordinates'))
    return fc(out),missing
...
def ensure_poc004():
    ...
    if not all((POC004/x).exists() for x in required):
        print('POC-004 derivatives missing; building baseline');build_poc004()
    else:print('POC-004 derivatives already exist; reusing locally validated baseline')
    OUT.mkdir(parents=True,exist_ok=True)
    for p in POC004.glob('*.geojson'):shutil.copy2(p,OUT/p.name)
    for p in POC004.glob('*.json'):
        if p.name not in {'manifest.json'}:shutil.copy2(p,OUT/p.name)
...
def main():
    ...
    (OUT/'manifest.json').write_text(json.dumps(layers,indent=2),encoding='utf-8');print('POC-005 complete')
